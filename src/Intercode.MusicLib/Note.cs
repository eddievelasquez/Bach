...
namespace Bach.Model
{
   ...
   public struct Note: IEquatable<Note>, IComparable<Note>
   {
      ...
      public const int INTERVALS_PER_OCTAVE = 12;
      ...
      {
         int midi;
         if( !Int32.TryParse(value, out midi) )
            return false;
      ...
      {
         Contract.Requires<ArgumentOutOfRangeException>(midi >= 0, "midi");
         Contract.Requires<ArgumentOutOfRangeException>(midi <= 127, "midi");

         int absoluteValue = midi - 12;
         if( absoluteValue < 0 )
            throw new ArgumentOutOfRangeException("midi is out of range");
